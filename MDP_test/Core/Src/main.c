...
#define SERVO_CENTER 152.2f // 156: Full 154.2f 154.6 old battery,
#define SERVO_CENTER_A 145
#define SERVO_CENTER_A_PERCENTAGE 30
#define SERVO_CENTER_B 154
#define SERVO_LEFT_MAX 100.6f  // 100.6
#define SERVO_RIGHT_MAX 208.6f // 208.6
...
//         HAL_UART_Transmit(&huart3, (uint8_t *)finalAck, strlen(finalAck), 0xFFFF);
//         return 1;
//     }
//
...
//     HAL_UART_Transmit(&huart3, (uint8_t *)buf3, strlen(buf3), 100);
//
//
//
//     return 0;
...
uint8_t motorPidForward(MotorCommand_t cmd, uint8_t isStateChanged)
{
    ...
    if (hasTargetDistance && totalDistanceA >= targetDistance - 1)
    {
        motorStop();
        isFrontCalib = 0;
    ...
    //    HAL_UART_Transmit(&huart3, (uint8_t*)debugBuf, len, 10);
...
uint8_t motorPidReverse(MotorCommand_t cmd, uint8_t isStateChanged)
{
    ...
    if (isStateChanged)
    {
        ...
        isFrontCalib = 0;
        isTurning = 0;
        setServoAngle(SERVO_CENTER + 2.5f); // Ensure wheels are straight
...
uint8_t motorTurnF(MotorCommandF_t cmd, uint8_t isStateChanged)
{
    ...
    if (remaining < 0.0f)
    {
        ...
        if (cmd.command == TURNL)
        {
            setServoAngle(SERVO_CENTER + 20.0f); // subjected to changes
            sprintf(buf1, "Target:  %.3f", targetAngle);
            sprintf(buf2, "Current: %.3f", currentAngle);
//          osDelay(200);
            osDelay(100);
...
uint8_t motorTurn90R(MotorCommand_t cmd, uint8_t isStateChanged)
{
    ...
    case POST_TURN:
        if (subStateChanged)
        {
            ...
            subCmd.param2DistAngle = 2.0; // Adjusted angle
        }
        if (motorPidReverseF(subCmd, subStateChanged))
        {
...
uint8_t RTURN90L_TEST(MotorCommand_t cmd, uint8_t isStateChanged)
{
    ...
    case POST_TURN:
        if (subStateChanged)
        {
            subCmd.command = REVS;
            subCmd.param1Speed = 1000;
            subCmd.param2DistAngle = 17.5f; // Adjusted angle
        }
        if (motorPidReverseF(subCmd, subStateChanged))
        {
            turnState = COMPLETED; // Jumps straight to completed
...
void rxSerialParse(void)
{
    ...
    if (cmdid < 0)
    {
        ...
        HAL_UART_Transmit(&huart3, result, strlen((char *)result), 0xFFFF);
        return;
    }
    cmd.cmdId = cmdid;
...
void irSensor(void *argument)
{
    ...
    for (;;)
    {
        ...
        // HAL_UART_Transmit(&huart3, (uint8_t*)ir_uart_buf, ir_lenR, 100);
        ...
        // HAL_UART_Transmit(&huart3, (uint8_t*)ir_uart_buf, ir_len_combined, 150);
